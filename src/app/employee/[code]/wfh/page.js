...
export default function WfhPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    const result = await submitWfhRequest(code, { date, reason: reason.trim() });
    setSubmitting(false);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', alignItems: 'start' }}>
      <div className="card" style={{ padding: '22px 24px' }}>
        <div style={{ fontSize: '15px', fontWeight: 700, letterSpacing: '-0.02em', marginBottom: '16px' }}>Apply for Work From Home</div>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            ...
            <textarea className="input-field" rows={3} placeholder="Brief reason for WFH…" value={reason}
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', fontSize: '14px', fontWeight: 700 }}>
          My WFH Requests ({wfhRequests.length})
        </div>
              ...
              <div key={r.id} style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <FiHome size={13} style={{ color: '#7b2d8b' }} />
                    <span style={{ fontSize: '13px', fontWeight: 600 }}>WFH</span>
